...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"LwYd":[function(require,module,exports) {
var define;
...
(function (mod) {
  ...
  var captureRightClick = gecko || ie && ie_version >= 9; // Optimize some code when these features are not used.
  ...
  function Display(place, doc) {
    ...
    d.inputDiv = elt("div", [input], null, "overflow: hidden; position: relative; width: 3px; height: 0px;"); // Covers bottom-right square when both scrollbars are present.

    d.scrollbarFiller = elt("div", null, "CodeMirror-scrollbar-filler");
    d.scrollbarFiller.setAttribute("not-content", "true"); // Covers bottom of gutter when coverGutterNextToScrollbar is on
    // and h scrollbar is present.

    d.gutterFiller = elt("div", null, "CodeMirror-gutter-filler");
    d.gutterFiller.setAttribute("not-content", "true"); // Will contain the actual code, positioned to cover the viewport.
    ...
    d.wrapper = elt("div", [d.inputDiv, d.scrollbarFiller, d.gutterFiller, d.scroller], "CodeMirror"); // Work around IE7 z-index bug (not perfect, hence IE7 not really being supported)
  ...
  function NativeScrollbars(place, scroll, cm) {
    ...
    this.checkedOverlay = false; // Need to set a minimum width to see the scrollbar on IE7 (but must not set it on IE8).
  ...
  function postUpdateDisplay(cm, update) {
    ...
    for (var first = true;; first = false) {
      if (first && cm.options.lineWrapping && update.oldDisplayWidth != displayWidth(cm)) {
        ...
        force = false; // Clip forced viewport to actual scrollable area.
  ...
  var lastCopied = null; // Read input from the textarea, and update the document to match.
  ...
  function readInput(cm) {
    ...
    if (text.charCodeAt(0) == 0x200b && doc.sel == cm.display.selForContextMenu && !prevInput) prevInput = "\u200b"; // Find the part of the input that is actually new
  ...
  function registerEventHandlers(cm) {
    ...
    on(d.input, "paste", function () {
      ...
      if (webkit && !cm.state.fakedLastChar && !(new Date() - cm.state.lastMiddleDown < 200)) {
        ...
        d.input.value += "$"; // The selection end needs to be set before the start, otherwise there
    ...
    on(d.input, "copy", prepareCopyCut); // Needed to handle Tab key in KHTML
  ...
  function onResize(cm) {
    var d = cm.display;
    if (d.lastWrapHeight == d.wrapper.clientHeight && d.lastWrapWidth == d.wrapper.clientWidth) return; // Might be a text scaling operation, clear size caches.
  ...
  function onFocus(cm) {
    ...
    if (!cm.state.focused) {
      ...
      addClass(cm.display.wrapper, "CodeMirror-focused"); // The prevInput test prevents this from firing when a context
  ...
  function onContextMenu(cm, e) {
    ...
    function prepareSelectAllHack() {
      if (display.input.selectionStart != null) {
        ...
        display.input.selectionEnd = extval.length; // Re-set this, in case some other handler touched the
  ...
  var bidiOrdering = function () {
    ...
    return function (str) {
      ...
      for (var i = 0, type; i < len; ++i) types.push(type = charType(str.charCodeAt(i))); // W1. Examine each non-spacing mark (NSM) in the level run, and
...
},{}],"WcLL":[function(require,module,exports) {
var define;
// CodeMirror, copyright (c) by Marijn Haverbeke and others
// Distributed under an MIT license: http://codemirror.net/LICENSE
(function (mod) {
  if (typeof exports == "object" && typeof module == "object") // CommonJS
    mod(require("../../codemirror.js"));else if (typeof define == "function" && define.amd) // AMD
    define(["../../codemirror.js"], mod);else // Plain browser env
    mod(CodeMirror);
})(function (CodeMirror) {
  ...
  CodeMirror.defineOption("autoCloseBrackets", false, function (cm, val, old) {
    ...
    if (typeof val == "string") pairs = val;else if (typeof val == "object") {
      ...
      map["'" + left + "'"] = function (cm) {
        ...
        for (var i = 0; i < ranges.length; i++) {
          ...
          if (!range.empty()) {
            ...
            if (cm.getRange(cur, Pos(cur.line, cur.ch + 3)) == left + left + left) curType = "skipThree";else curType = "skip";
          } else if (left == right && cur.ch > 1 && cm.getRange(Pos(cur.line, cur.ch - 2), cur) == left + left && (cur.ch <= 2 || cm.getRange(Pos(cur.line, cur.ch - 3), Pos(cur.line, cur.ch - 2)) != left)) {
            curType = "addFour";
          } else if (left == '"' || left == "'") {
            if (!CodeMirror.isWordChar(next) && enteringString(cm, cur, left)) curType = "both";else return CodeMirror.Pass;
...
},{"../../codemirror.js":"LwYd"}]},{},["WcLL"], null)
//# sourceMappingURL=/closebrackets.03b28edf.js.map